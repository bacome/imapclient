...
{
    public abstract class cMessageDataPart
    {
        public readonly fMessageDataFormat Format;

        internal cMessageDataPart(fMessageDataFormat pFormat)
        {
            Format = pFormat;
        }
    ...
    public class cLiteralMessageDataPart : cMessageDataPart
    {
        ...
        internal cLiteralMessageDataPart(cBytes pBytes, fMessageDataFormat pFormat) : base(pFormat)
        {
            Bytes = pBytes ?? throw new ArgumentNullException(nameof(pBytes));
        }

        public cLiteralMessageDataPart(IEnumerable<byte> pBytes, fMessageDataFormat pFormat) : base(pFormat)
        {
...
;?; // this is n
    public abstract class cAppendDataPartx
        {
            public static readonly cAppendDataPart CRLF = new cLiteralAppendDataPart("\r\n");

            internal cAppendDataPart() { }
            internal abstract bool HasContent { get; }
            public static implicit operator cAppendDataPart(cIMAPMessage pMessage) => new cMessageAppendDataPart(pMessage);
            public static implicit operator cAppendDataPart(cIMAPAttachment pAttachment) => new cMessagePartAppendDataPart(pAttachment);
