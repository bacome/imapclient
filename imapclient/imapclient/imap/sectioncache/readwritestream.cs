...
namespace work.bacome.imapclient
{
    ...
    {
        internal sealed class cReaderWriter : IDisposable
        {
            ...
            public cReaderWriter(cSectionCacheItem pItem, cSectionCachePersistentKey pKey, cBatchSizer pWriteSizer, cTrace.cContext pParentContext)
            {
            ...
            public cReaderWriter(cSectionCacheItem pItem, cSectionCacheNonPersistentKey pKey, cBatchSizer pWriteSizer, cTrace.cContext pParentContext)
            {
                mItem = pItem ?? throw new ArgumentNullException(nameof(pItem));
                mPersistentKey = null;
            ...
            public long ReadPosition => mReadPosition;

            public async Task SetReadPosition(cMethodControl pMC, long pReadPosition, cTrace.cContext pParentContext)
            {
                var lContext = pParentContext.NewMethod(nameof(cReaderWriter), nameof(SetReadPosition), pMC, pReadPosition);
            ...
            public async Task WriteEndAsync(cTrace.cContext pParentContext)
            {
                ...
                {
                    if (mNonPersistentKey.UID == null) mItem.mCache.Add(mNonPersistentKey, mItem, lContext);
                    else mItem.mCache.Add(new cSectionCachePersistentKey(mNonPersistentKey), mItem, lContext);
                }
                else mItem.mCache.Add(mPersistentKey, mItem, lContext);
            }
