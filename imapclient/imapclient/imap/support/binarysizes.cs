    ...
    public class cBinarySizes : ReadOnlyDictionary<string, uint>
    {
        ...
        private void OnDeserialised(StreamingContext pSC)
        {
            foreach (var lKey in Keys)
            {
                ...
                if (!cMailValidation.IsSectionPart(lKey)) throw new cDeserialiseException(nameof(cBinarySizes), nameof(Keys), kDeserialiseExceptionMessage.ContainsInvalidValues);
            }
        }
