...
namespace work.bacome.imapclient
{
    ...
    {
        private class cFetchSizer
        {
            public readonly cFetchSizeConfiguration Configuration;
            private readonly cSamples[] mSamples = new cSamples[4];
            private int _Current;
            private readonly double mMaxTime;

            public cFetchSizer(cFetchSizeConfiguration pConfiguration)
            {
            ...
            public override string ToString() => $"{nameof(cFetchSizer)}({Configuration},{_Current})";
            ...
            {
                var lContext = pParentContext.NewMethod(nameof(cFetchSizer), nameof(_Tests));

                cFetchSizer lSizer = new cFetchSizer(new cFetchSizeConfiguration(1, 100, 1000, 10));

                if (lSizer.Current != 10) throw new cTestsException($"{nameof(cFetchSizer)}.1.1");
                lSizer.AddSample(10, 1000, lContext);
                if (lSizer.Current != 10) throw new cTestsException($"{nameof(cFetchSizer)}.1.2");
                lSizer.AddSample(5, 500, lContext);
                if (lSizer.Current != 10) throw new cTestsException($"{nameof(cFetchSizer)}.1.3");
                lSizer.AddSample(150, 1500, lContext); // 10 times faster
                if (lSizer.Current != 20) throw new cTestsException($"{nameof(cFetchSizer)}.1.4");
                lSizer.AddSample(235, 1000, lContext);
                if (lSizer.Current != 40) throw new cTestsException($"{nameof(cFetchSizer)}.1.5");
                lSizer.AddSample(10, 100, lContext);
                if (lSizer.Current != 80) throw new cTestsException($"{nameof(cFetchSizer)}.1.6");
                lSizer.AddSample(10, 100, lContext);
                if (lSizer.Current != 100) throw new cTestsException($"{nameof(cFetchSizer)}.1.7");
                lSizer.AddSample(10, 100, lContext);
                if (lSizer.Current != 100) throw new cTestsException($"{nameof(cFetchSizer)}.1.8");
                lSizer.AddSample(1, 1000, lContext);
                if (lSizer.Current != 1) throw new cTestsException($"{nameof(cFetchSizer)}.1.9");
                lSizer.AddSample(10, 1000, lContext);
                if (lSizer.Current != 2) throw new cTestsException($"{nameof(cFetchSizer)}.1.10");
                lSizer.AddSample(10, 1000, lContext);
                if (lSizer.Current != 4) throw new cTestsException($"{nameof(cFetchSizer)}.1.11");
                lSizer.AddSample(10, 1000, lContext);
                if (lSizer.Current != 8) throw new cTestsException($"{nameof(cFetchSizer)}.1.12");
                lSizer.AddSample(10, 1000, lContext);
                if (lSizer.Current != 8) throw new cTestsException($"{nameof(cFetchSizer)}.1.13");
                lSizer.AddSample(10, 1000, lContext);
                if (lSizer.Current != 10) throw new cTestsException($"{nameof(cFetchSizer)}.1.14");
                lSizer.AddSample(100, 1000, lContext);
                if (lSizer.Current != 20) throw new cTestsException($"{nameof(cFetchSizer)}.1.15");
                lSizer.AddSample(100, 1000, lContext);
                if (lSizer.Current != 33) throw new cTestsException($"{nameof(cFetchSizer)}.1.16");
                ...
                lSizer = new cFetchSizer(new cFetchSizeConfiguration(1000, 1000000, 10000, 1000));
