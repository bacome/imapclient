...
namespace work.bacome.imapclient
{
    internal class cSASLXOAuth2 : cSASL
    {
        ...
        {
            if (string.IsNullOrEmpty(pUserId) || pUserId.IndexOf(kChar.CtrlA) != -1) throw new ArgumentOutOfRangeException(nameof(pUserId));
            ...
            public override IList<byte> GetResponse(IList<byte> pChallenge)
            {
                switch (mState)
                {
                    case eState.unchallenged:
                        ...
                        string lResponse = "user=" + mUserId + kChar.CtrlA + "auth=Bearer " + mAccessToken + kChar.CtrlA + kChar.CtrlA;
