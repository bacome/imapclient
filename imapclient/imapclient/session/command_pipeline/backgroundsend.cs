                ...
                private void ZBackgroundSendDequeueAndAppendTag(cTrace.cContext pParentContext)
                {
                    ...
                    lock (mPipelineLock)
                    {
                        while (mQueuedCommands.Count > 0)
                        {
                            ...
                            {
                                if (lCommand.UIDValidity != null && lCommand.UIDValidity != mMailboxCache?.SelectedMailboxDetails?.MessageCache.UIDValidity) lCommand.SetException(new cUIDValidityChangedException(lContext), lContext);
                                else
                                {
